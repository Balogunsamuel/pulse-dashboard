...
function SecurityTabs({ groupId }: { groupId: string }) {
  const [activeTab, setActiveTab] = useState<'spam' | 'raid' | 'scam' | 'live'>('spam');
  ...
  return (
    <div>
      ...
      <div className="p-6">
        ...
        {activeTab === 'scam' && <ScamPatternsTab />}
...
function ScamPatternsTab() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [patternsData, statsData] = await Promise.all([
        api.getScamPatterns(false),
        api.getScamStats(),
      ]);
      setPatterns(patternsData);
      setStats(statsData);
    } catch (err) {
      console.error('Error fetching scam patterns:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggle = async (patternId: string) => {
    try {
      await api.toggleScamPattern(patternId);
      fetchData(); // Refresh data
    } catch (err) {
  ...
  const handleDelete = async (patternId: string) => {
    ...
    try {
      await api.deleteScamPattern(patternId);
      fetchData(); // Refresh data
    } catch (err) {
  ...
  if (loading) {
    return <div className="text-gray-500">Loading scam patterns...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-xl font-bold text-orange-600">{stats.bySeverity.high}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-sm text-gray-600">Medium</p>
            <p className="text-xl font-bold text-yellow-600">{stats.bySeverity.medium}</p>
          </div>
        </div>
      )}
      ...
      {/* Patterns List */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Scam Detection Patterns</h3>
        {patterns.length === 0 ? (
          <p className="text-gray-500">No patterns configured</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
...
function AddPatternModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      await api.addScamPattern({ pattern, type, severity, description });
      onSuccess();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Add Scam Pattern</h3>
